...
class Dataset:
    def __init__(self, dataset_dir, hparams=None, training=True):
        ...
        print(self.src_max_len)
        self.tgt_max_len = self.hparams.tgt_max_len
        print(self.tgt_max_len)
        self.dataset = None
    ...
    def get_training_batch(self, num_threads=4):
        ...
        train_set = train_set.map(lambda src, tgt:
                                  (src, tf.concat(([self.hparams.sos_id], tgt), 0),
    ...
    def get_inference_batch(self, src_dataset, batch_size):
        infer_dataset = src_dataset.map(lambda src: tf.string_split([src]).values)

        infer_dataset = infer_dataset.map(lambda src: src[:self.hparams.src_max_len_infer])
        infer_dataset_ids = infer_dataset.map(lambda src: tf.cast(self.src_vocab_table.lookup(src),
                                                                  tf.int32))

        infer_dataset_ids = infer_dataset_ids.map(lambda src: (src, tf.size(src)))
    ...
    def _load_dataset(self, dataset_dir):
        src_dataset = tf.data.TextLineDataset([os.path.join(dataset_dir, 'train.enc')])
        tgt_dataset = tf.data.TextLineDataset([os.path.join(dataset_dir, 'train.dec')])
