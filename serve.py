...
class Serve:
    ...
    def __init__(self, sess, model_name, checkpoint):
        os.makedirs(os.path.join('data', 'test', model_name), exist_ok=True)
        data_dir = os.path.join('data', 'train', model_name,'dataset')
        model_dir = os.path.join('model', model_name)
        hparams = seq2seq.utils.load_hparams(os.path.join('tn', 'char_level_hparams.json'))
